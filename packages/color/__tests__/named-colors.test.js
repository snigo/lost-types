...
test('correct format of parseNamed output', () => {
  ...
  expect(namedColors.values().every((value) => value.length === 7)).toBe(true);
});
...
test('returns empty array if did not parsed', () => {
  expect(parseNamed()).toEqual([]);
  expect(parseNamed(null)).toEqual([]);
  expect(parseNamed('')).toEqual([]);
  expect(parseNamed('creamson')).toEqual([]);
  expect(parseNamed('meganta')).toEqual([]);
  expect(parseNamed('rain')).toEqual([]);
});
